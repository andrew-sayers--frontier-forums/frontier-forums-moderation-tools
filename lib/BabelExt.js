  ...
  return { // public interface
    ...
    utils: {
      ...
      runInEmbeddedPage: function(content) {
          /* addons.mozilla.org reviewers - please read this
           *
           * This function has been rejected by review in the past - that's fine, I'm happy to fix the issue.
           * But I haven't been able to get enough information either by e-mail or IRC to do any more than cargo cult a solution.
           * I don't have enough free time to continue chasing, so am writing my request here.
           * Next time you reject this function, please answer the following questions:
           *
           * 1) What is the recommended way to inject code into the embedded page, for example to respond to a JSONP request or interact with code on the page?
           * 2) If there's no specific recommendation, what would be a good example of a security issue I need to protect against?
           * 3) Is there anything else I can do to make this code more readable for you guys?  Rename it?  Add documentation?  Link to some website?
           *
           * Thanks :)
           */
        var script = document.createElement('script'); // addons.mozilla.org reviewers - please see note above this line
      ...
      this.open = function(method, url, async, user, password) {
        ...
        if ( url == location.origin || url.search(location.origin+'/') == 0 || url.search(/^[a-z]+:/) == -1 ) {
          ...
          this.send = function(data) {
            if ( BabelExt._xhr_escape && typeof(data) == 'string' ) data = data.replace( /(?:%[89ABCDEF][0-9A-F])+/g, function(match) { return escape(decodeURIComponent(match)) });
