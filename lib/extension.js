BabelExt.utils.dispatch(
     ...
     */
    {
        ...
        callback: function( stash, pathname, params, infractions ) {
            ...
            stash.thread_edit = function( thread_id, title, notes, prefix, close_thread ) {
                return stash.post_message(
                    '/postings.php?do=updatethread&t=' + thread_id,
                    {
                        'do': 'updatethread',
...

    {
        ...
        callback: function( stash, pathname, params, infractions, infractions_timestamp, reload_interval ) {
                ...
                $.ajax({
                    ...
                    success: function(html) {
                        var infractions = $(html).find('input[name="infractionlevelid"]').map(function() {
                            if ( $(this).val() != '0' )
                                return {
                                    name: $(this).parent().text().replace(/^\s*|\s*$/g,''),
                                    id: $(this).val(),
            ...
            stash.give_infraction = function( note, message, user_id, post_id, infraction_id, is_warning ) {
                    var data = {
                        'do': 'update',
                        note: note,
                        message: message,
     ...
     */
    {
        pass_storage: ['variables','variables_timestamp'],
        pass_preferences: [ 'variable_thread_id', 'reload_interval' ],
        callback: function( stash, pathname, params, variables, variables_timestamp, variable_thread_id, reload_interval ) {

            variables = JSON.parse(variables||'{}');

            if ( parseInt(variables_timestamp||'0',10)+reload_interval*1000 < new Date().getTime() ) {
                stash.get_thread_posts( variable_thread_id )
                    .then(
                    function(posts) {
                        return $.when.apply(
                            $,
     ...
     */
    {
        ...
        callback: function(stash, pathname, params) {
            ...
            $.get( '/postings.php?do=editthread&t=' + params.t, function(html) {
                ...
                ;
                html.find('div.vbform .blockhead').wrapInner( '<a href="/postings.php?do=editthread&t=' + params.t + '"></a>' );
                stash.report_block.filter('div.vbform').replaceWith( html.find('div.vbform') );
            });
            stash.report_block.find('#threadbits_forum_48_container').load( '/forumdisplay.php?f=48 #threads', function() {
                ...
                stash.report_block.find('.prefix_closed,.prefix_deleted,.prefix.understate span[style="color:grey;"]').closest('li').remove();
...

    {
        ...
        callback: function(stash, pathname, params, stylesheet) {
            $(stylesheet)
                .clone()
                .attr( 'href', stylesheet.href.replace( /([?&]sheet)=[^&]*/, '$1=threadlist.css' ))
    ...
    { // prepare to merge threads
        ...
        callback: function(stash, pathname, params, merge_data, reload_interval) {
            ...
            stash.report_merge = function( current_thread, destination_forum,  destination_thread,  destination_title ) {
                var dfd = new jQuery.Deferred();
                stash.thread_edit( current_thread, undefined, undefined, undefined, true ) // close thread
            ...
            stash.get_variable('frequent merge destinations').replace(/[0-9]+/g, function(thread_id) {
                ...
                if ( merge_data.hasOwnProperty(thread_id) && parseInt(merge_data[thread_id].timestamp,10)+reload_interval*1000 < new Date().getTime() ) {
                    ...
                    promise = $.get( '/showthread.php?t=' + thread_id, function(html) {
                        merge_data[thread_id] = {
                            timestamp: new Date(),
                            forum_id: html.find( '#breadcrumb .navbit a').last().attr('href').split( '?f=' )[1],
                            title: html.find( '#breadcrumb .navbit.lastnavbit' ).text().replace( /^\s*|\s*$/g, '' )
                        };
                        return [ merge_data[thread_id].forum_id, merge_data[thread_id].title ];
                    });
                    get_requests.push(promise);
                }
        ...
        callback: function(stash, pathname, params, threadtools, merge_data, reload_interval) {
            ...
            var quick_merge = $(
                    '<li class="popupmenu">' +
                      ...
                      '<ul style="min-width: 350px" class="popupbody"></ul>' +
                    '</li>'
            ).insertBefore(threadtools);

            stash.merge_destinations.forEach(function(destination) {

                var link = $('<li><a rel="nofollow" href="#merge-thread">Merge with thread #' + destination.thread_id + '</a></li>')
                    .appendTo(quick_merge.find('.popupbody'))
                ...
                destination.promise.done(function( forum_id, title ) {
                    link.find('a').text( 'Merge with ' + title );
                });
...

    {
        ...
        callback: function( stash, pathname, params, below_postlist) {
            stash.report_block.insertBefore( below_postlist );
            ...
            $("head").append(
                "<style type='text/css'>" +
                    ...
                    '.report-menu:hover .memberaction_body, .report-menu:hover .memberaction_body li { display: block }' +
                "</style>"
            );
            ...
            stash.process_posts().each(function() {
                ...
                this.report_element
                    .wrap('<div class="report-menu"></div>').parent().append(
...

    {
        ...
        match_elements: 'input[name="mergethreadurl"]',
    ...
    },
    {
        ...
        callback: function(stash, pathname, params) {
            ...
            $('select[name="period"]').val( 5 );
            $('select[name="frame"]').val( 'd' );
     ...
     */
    {
        ...
        callback: function(stash, pathname, params) {
            ...
            var first_post = stash.process_posts()[0],
                user_to_review = first_post.message_element.find('a[href^="' + location.origin + '/member.php"]').eq(1),
            ...
            var reviewed_user_info = $('<div>' + stash.loading_html + ' information about ' + user_to_review.html() + '</div>').appendTo( first_post.message_element );
            ...
            $.when(
                ...
                reviewed_user_info.html(
                    ...
                    '<ul id="infractionslist"><li><b>Infractions</b>: none</ul>' +
                ...
                $('#view-stats_mini').find('.userinfo').css({ width: 'inherit' });
                if ( member.find('#infractionslist').length ) {
                    $('#infractionslist').replaceWith( member.find('#infractionslist').css({ clear: 'both' }) );
                ...
                if ( notes.length ) {
                    $('#user_notes').replaceWith( notes.attr( 'id', 'user_notes' ).css({ clear: 'both' }) );
                    ...
                    $('#user_notes').find('h2.title').each(function() {
                        $(this).text().replace( pm_worthy_re, function(match) {
                            delete pm_worthy_violations[ pm_worthy_violations.toLowerCase() ];
                        });
                    });
                }
            ...
            case 'yours':
                ...
                if ( document.title.search( expected_title_suffix ) == -1 )
                    stash.thread_edit( params.t, document.title.replace(/ - .*? on it$/) + expected_title_suffix );
                handle_thread();
                break;
            case 'taken':
            ...
            function handle_thread() {
                ...
                $("head").append(
                    "<style type='text/css'>" +
                        ...
                        '.common-actions #submit-resolution { background: none; padding: 0 }' +
                        ...
                        '.common-actions .notification.none .none,.common-actions .notification.pm .pm,.common-actions .notification.warn .warn,.common-actions .notification.infract .infract { display: inherit }' +
                      ...
                      '<div class="column2">' +
                        '<fieldset class="notification none vbcode-mode">' +
                          ...
                          '<div style="text-align: center; margin-bottom: 1em" class="posthead">' +
                            ...
                            make_input('radio', 'notification-type', 'pm', 'Send private message' ) + ' ' +
                            ...
                            make_input('radio', 'notification-type', 'warn', 'Give official warning' ) + ' ' +
                            make_input('radio', 'notification-type', 'infract', 'Give infraction' ) + ' ' +
                          '</div>' +
                          '<div class="none">' +
                ...
                common_actions.find('.postcontrols').html('<a id="submit-resolution" href="#submit-resolution"></a>');
                ...
                function refresh_post_contents() {
                    return posts[current_post].vbcode_promise.done(function(text) {
                        if ( posts[current_post].action == 'edit' ) {
                            $('.common-actions .search-for').each(function() {
                                if ( $(this).val() == '' ) return;
                                var replacement = replacement = $(this.nextSibling).val();
                                text = text.replace(
                                    new RegExp( $(this).val().replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g, "\\$&"), 'gi' ),
                                    function(match) {
                                        return '\uE001' + match + '\uE002' + replacement + '\uE003'; // characters from Unicode's private use area
                                    }
                                );
                            });
                            posts[current_post].replaced_vbcode = text.replace(/\uE001(?:.|\n)*?\uE002((?:.|\n)*?)\uE003/g, '$1' );
                        } else if ( posts[current_post].action == 'hand-edit' && posts[current_post].hasOwnProperty('edited_vbcode') ) {
                            posts[current_post].replaced_vbcode = posts[current_post].edited_vbcode;
                        } else {
                            posts[current_post].replaced_vbcode = text;
                        }
                        ...
                        $('.per-post div.vbcode').html(
                            BabelExt.utils.escapeHTML(text).replace(/\uE001/g,'<del>').replace(/\uE002/g,'</del><ins>').replace(/\uE003/g,'</ins>')
                        );
                    });
                }
                ...
                stash.review_post_promise.done(function(html) {
                    ...
                    stash.get_thread_posts( stash.thread_to_review_id, html ).done(function(more_posts) {
                        posts = posts.concat( more_posts.get().slice(posts.length) );
                ...
                form.find( 'input[name="issue-type"]' ).click(function() {
                    var issue_name = $(this).parent().text().replace(/^\s*|\s*$/g,'');
                    if ( pm_worthy_violations.hasOwnProperty(issue_name.toLowerCase()) )
                        form.find( 'input[name="notification-type"][value="pm"]' ).click();
                    else if ( infraction_worthy_violations.hasOwnProperty(issue_name.toLowerCase()) || infraction_count >= 3 )
                        form.find( 'input[name="notification-type"][value="infract"]' ).click();
                    else
                        form.find( 'input[name="notification-type"][value="warn"]' ).click();
                    $('#issue-info').html( stash.get_variable([ 'information', issue_name ]) );
                    $('.issue-name').text(issue_name);
                    $('.issue-points').text($(this).data('points'));
                ...
                form.find('input[name="issue-type"],input[name="notification-type"],#delete-posts,input[name="action"]').change(function() {
                    ...
                    stash.review_post_promise.done(function() {
                        ...
                        var issue_name = $('input[name="issue-type"]:checked').parent().text().replace(/^\s*|\s*$/g,'');
                        ...
                        case '':
                            ...
                            break;
                        case 'pm':
                            $('.notification .pm input'   ).val( stash.get_variable([ 'PM title', issue_name ], variables ) );
                            $('.notification .pm textarea').val( stash.get_variable([ 'PM body' , issue_name ], variables ) );
                            notification_data = {
                        ...
                        case 'infract':
                            ...
                            $('.notification .' + notification_type + ' textarea').val(
                                stash.get_variable([ 'infraction', issue_name ], variables )
                            );
                            notification_data = {
                                description: variables['infraction type'],
                                get_promises: function() {
                                    return [stash.give_infraction(
                                        'See ' + document.location.toString(), // administrative note
                                        $('.notification .' + notification_type + ' textarea').val(),
                                        user_to_review_id,
                                        post_to_review_id,
                                        $('.common-actions input[name="issue-type"]:checked').val(),
                                        is_warning
                                    )];
                                },
