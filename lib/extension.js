BabelExt.utils.dispatch(
            ...
            stash.get_member_info = function(user_id) {
                return $.get('/member.php?u='+user_id+'&tab=infractions&pp=50').then(function(html) {
                    ...
                    if ( html.find('.infractions_block').length ) {
                        ...
                        ret.infraction_summary = infractions.closest('li').map(function() {
            ...
            stash.build_user_report = function( username_html, username_text, user_id, container ) {
                ...
                modcp_promise.progress(function() {
                    ...
                    $.get( '/modcp/user.php?do=doips', function(html) {
                        ...
                        $.post( '/modcp/user.php?do=doips', $.extend( post_args, { username: username_text } ), function(html) {
                            var known_names = {};
                            var promise_count = 1;
                            ...
                            function process_search_page(html, ul, depth) {
                                return $(html).find( '#cpform_table ul ul > li' ).map(function() {
                                    ...
                                    if ( known_names.hasOwnProperty(user_id) ) {
                                        ...
                                        ++promise_count;
                                        infraction_data[user_id] = $.when(
                                            $.get( '/member.php?tab=infractions&pp=50&u=' + user_id ),
                                            $.get( '/modcp/user.php?do=viewuser&u=' + user_id )
                                        ).then(function(member, modcp) {
                                            resolve_promise();
                                            member = $(member[0]);
                                            var email = BabelExt.utils.escapeHTML($(modcp[0]).find('input[name="user\\[email\\]"]').val());
                                            return [
                                                ' &lt;<a href="mailto:' + email + '">' + email + '</a>&gt;' +
                                                ' - joined ' + BabelExt.utils.escapeHTML(member.find( '.userinfo dd' ).first().text() + ', ' + member.find('#userinfo .usertitle').text()),
                                                member
                                    ...
                                    var li = $('<li style="margin-left:1em; list-style:disc"></li>')
                                        ...
                                        .append(
                                            '<span></span> - check on: ' +
                                            '<a class="stopforumspam" href="//api.stopforumspam.org/api?username=' + encodeURIComponent(name) + '&amp;ip=' + encodeURIComponent(address) + '&amp;f=json">StopForumSpam.com</a>' +
                                            ', ' +
                                            '<a href="http://multirbl.valli.org/lookup/' + encodeURIComponent(address) + '.html">MultiRBL.valli.org</a>'
                                        )

                                    ;
                                    li.find('.stopforumspam').click(function(event) {
                                        stash.check_on_stopforumspam( this.href );
                                        event.preventDefault();
                                    });
                                    infraction_data[user_id].then(function(data) {
                                        li.find('span').html( data[0] );
                                        li.prepend(data[1]);
                                    });
                                    return [ [ name, li ] ];
                                });
                                ...
                                if ( ul.children().length ) {
                                    container.find('#duplicate-account-info')
                                        ...
                                        .append(
                                            ...
                                            '</div>' +
                                            '<div id="user-info-email-address" style="margin: 0.5em 0" >'+stash.loading_html+' e-mail address</div>'
                                        )
                                        .append(ul);
                                    container.find('#duplicate-account-info input[type="button"]').click(function() {
                                        $(this).siblings('div').toggle();
                                    });
                                    modcp_promise.done(function(iframe) {
                                        $('#user-info-email-address').html(
                                            'E-mail address for <i>' + username_html + '</i>: ' +
                                            BabelExt.utils.escapeHTML( $(iframe.contentDocument.body).find('input[name="user\\[email\\]"]').val() )
                                        );
                                    });
                                } else {
                ...
                var userinfo_promise = $.when(
                    ...
                    modcp_promise,
                    $.get('/member.php?u='+user_id+'&tab=infractions&pp=50') // ensure the correct protocol is used
                    ...
                    member = $(member[0]);
                    container.find('.view-stats_mini').replaceWith( member.find('#view-stats_mini').removeAttr( 'id' ).addClass( 'view-stats_mini' ) );
        ...
        callback: function( stash, pathname, params, variables, variables_origin, variables_timestamp, variable_thread_id, reload_interval ) {
            ...
            function update_variables(posts) {
                ...
                return $.when.apply(
                    ...
                    var min_variables_version = 1, max_variables_version = 1, current_version = variables['variables version']||0;
                    if ( current_version < min_variables_version ) {
                        alert( stash.get_variable( 'error: version too low', {'variable thread id': variable_thread_id, 'min variables version': min_variables_version } ) );
                    }
                    if ( current_version > max_variables_version ) {
                        alert( stash.get_variable( 'error: version too high', {'variable thread id': variable_thread_id, 'max variables version': max_variables_version } ) );
                    }
            ...
            stash.get_variable = function( names, keys ) {
                ...
                if ( !variables.hasOwnProperty(name.toLowerCase()) ) return name;
                if ( !keys ) keys = {};
            ...
            if ( parseInt(infractions_timestamp||'0',10)+reload_interval*1000 < new Date().getTime() ) {
                stash.variables_promise.done(function() {
                    $.ajax({
                        ...
                        dataType: 'html',
                        success: function(html) {
                            var missing_infraction_text = '', infraction_map = {};
                            ...
                            var bad_infractions = stash.get_array_variable('infraction-worthy violations').filter(function(violation) { return !infraction_map.hasOwnProperty(violation) });
                            if ( bad_infractions.length && confirm(
                                "Some infraction-worthy violations do not exist.  Please fix the following:\n\n" +
                                    bad_infractions.join("\n") + "\n\n" +
                                    "To fix these names, paste the above into a text editor then go to the variables thread and change \"infraction-worthy violations\".\n" +
                                    "\n" +
                                    "Would you like to go there now?"
                            )) {
                                location = '/showthread.php?' + variable_thread_id;
                            ...
                            var bad_pms = stash.get_array_variable('PM-worthy violations').filter(function(violation) { return !infraction_map.hasOwnProperty(violation) });
                            if ( bad_pms.length && confirm(
                                "Some PM-worthy violations do not exist.  Please fix the following:\n\n" +
                                    bad_pms.join("\n") + "\n\n" +
                                    "To fix these names, paste the above into a text editor then go to the variables thread and change \"pm-worthy violations\".\n" +
                                    "\n" +
                                    "Would you like to go there now?"
                            )) {
                                location = '/showthread.php?' + variable_thread_id;
                ...
                $('<a style="float: right" title="click here to send a generic &ldquo;welcome to the forums&rdquo; message" href="#welcome-response">send "welcome" response</a>')
                    ...
                    .click(function(event) {
                        ...
                        stash.thread_reply(
                            ...
                            stash.get_variable('post title: welcome', { 'thread starter': thread_starter }),
                            stash.get_variable( 'post body: welcome', { 'thread starter': thread_starter })
                        )
     ...
     */
    {
        ...
        callback: function(stash, pathname, params) {
            ...
            case 'yours':
                var expected_title_suffix = stash.get_variable('report title suffix', { moderator: $('.welcomelink a').text() });
                if ( document.title.search( expected_title_suffix.replace(/([.*+?^${}()|\[\]\/\\])/g, "\\$1") ) == -1 )
                    stash.thread_edit(
                        params.t,
                        document.title.replace(
                            new RegExp(
                                stash.get_variable('report title suffix', { moderator: "\uE000" })
            ...
            function handle_thread() {
                ...
                var foreground_colour = $('.welcomelink').css( 'color' );
                            ...
                            useraction_data = {
                                ...
                                get_promises: function() {
                                    ...
                                    var args = [
                                        ...
                                        stash.get_variable([ 'ban reason', issue_count, issue_name ], variables ),
                                        textarea.val(),
                                        user_to_review_id,
                                        post_to_review_id,
                                        is_warning,
