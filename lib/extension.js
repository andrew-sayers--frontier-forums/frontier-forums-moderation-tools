(function(localStorage) { // note: we pass in a 'localStorage' variable because Firefox extensions can't use the real localStorage directly
    ...
    // Somehow, a bad release caused this variable to be set to "null" once:
    if (template_post_overrides === null) template_post_overrides = {};
    ...
    if (
        ...
        if ( $('#collapseimg_newpost_options').length )
            window.scrollTo( 0, $('#collapseimg_newpost_options').offset().top - $(window).height() );
    }
    ...
    if ( // Member's ModCP page
        ...
        $('a[href^="user.php"]').each(function() {
            $('<a class="stopforumspam">search on StopForumSpam.com</a>')
                // convert a username to a StopForumSpam API URL (e.g. <a href="user.php?do=viewuser&u=12345">Joe Bloggs</a> -> ...?username=Joe%20Bloggs
                .attr( 'href', 'http://api.stopforumspam.org/api?f=json&username=' + encodeURIComponent($(this).text()) )
    ...
    $('#last_post_container').load( 'http://forums.frontier.co.uk/showthread.php?t=47517&goto=newpost [id^=edit]:last', function() {
    ...
    if ( $('table.tborder').eq(0).has( 'a[href="forumdisplay.php?f=48"]' ).length ) {
        ...
        if ( posts.length == 1 )
            ...
            '<li><a href="/newreply.php?t=47517">Copy an appeal to the moderation log</a>' +
