BabelExt.utils.dispatch(
            ...
            stash.get_member_info = function(user_id) {
                return $.get('/member.php?u='+user_id+'&tab=infractions&pp=50').then(function(html) {
                    ...
                    var ret = {
                        ...
                        infraction_count  : 0,
                        warning_count     : 0,
                        infraction_summary: html.find('#usermenu a[href^="modcp/banning.php?do=liftban"]').length ? '<span style="color: red">BANNED&nbsp;</span>' : ''
                    };
     ...
     */
    {
        ...
        callback: function(stash) {
            ...
            $('#threads_table input[type="radio"],#posts input[type="radio"]')
                .change(function() {
                    if ( $(this).hasClass('spambot') ) {
                        $('.user-'+$(this).closest('td').data('user_id') + ' .spambot').click();
                    } else {
                        $('.user-'+$(this).closest('td').data('user_id') + ' input[value='+this.value+']').not('.spambot').click();
                    }
                });
            ...
            $('#submit0,#submit1').click(function(event) {
                var promises = [], known_users = {}, form = $(this).closest('form');
                $( (this.id=='submit0'?'#threads':'#posts') + '_table input.spambot:checked').each(function() {
     ...
     */
    {
        ...
        callback: function(stash, pathname, params) {
            ...
            var memberlist_pageno = 0, memberlist = stash.report_block.find('#memberlist');
            function get_memberlist(html) {
                ...
                html.find('#memberlist_table tr:not(.columnsort)').each(function() {
                    if ( $(this).children('td').filter(function() { return $(this).text().search( /^(?:Today|Yesterday)$/ ) != -1 }).length == 0 ) {
                        ...
                        memberlist.append($(this).find('a.username'));
                    }
                });
