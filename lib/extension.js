(function(localStorage) { // note: we pass in a 'localStorage' variable because Firefox extensions can't use the real localStorage directly
        ...
        template_posts = [ // we use an array here to control the order they're displayed in
...

            {
                ...
                body: "<<PM_HEADER>>\n\n" +
                    ...
                    "To check for filtered words before posting, click the [i]Preview Post[/i] button on the post page or the [i]Go Advanced[/i] button in the quick reply box.  If you don't spot a filtered word until your post is live, you can remove it with the [i]edit[/i] button.\n\n" +
                    "[post=<<POST_ID>>]Your message[/post] has now been edited to comply with forum rules, and you can see the changes on [URL=\"http://forums.frontier.co.uk/posthistory.php?p=<<POST_ID>>\"]its \"last edited\" page[/URL].  If you re-edit the post to avoid the problem phrase altogether, please notify us by replying to this message.  That way we know you're just tidying up, not trying to avoid moderation.\n\n" +
                    "For your reference, the rules on inappropriate language are quoted below.\n\n" +
                    "<<LANGUAGE RULES>>\n\n" +
                    "<<PM_FOOTER>>",
                copy_to_user_note: 1
            },
        ...
        function() {
            ...
            $('<tr><td class="vbmenu_option vbmenu_option_alink"><a href="http://api.stopforumspam.org/api?ip=' + $('img[alt=IP]', this).attr('title') + '&username=' + encodeURIComponent(username) + '&f=json">Search on StopForumSpam.com</a></td></tr>')
                ...
                .find( 'a' ).click(function(event) {
                    $.ajax({ url: $(this).attr('href'), dataType: "jsonp", jsonpCallback: 'stop_forum_spam_callback' });
                    event.preventDefault();
                });
            ;
    ...
    if ( $('table.tborder').eq(0).has( 'a[href="forumdisplay.php?f=48"]' ).length ) {
            ...
            if ( reported_user != userid ) { // people sometimes report their own posts
                ...
                var notes =
                    $('<div style="border-left: 3px solid black; padding-left: 1em">' +
                      ...
                      '<div class="infractions"></div>' +
                      '<br>' +
                      '<div class="user-notes"></div></div>'
                    )
                ...
                $.ajax({
                    ...
                    success: function(html) {
                        ...
                        if ( $infractions.find('tr').length > 1 ) {
                            ...
                            $infractions.html( '(no infractions)' ).prev().remove();
                        }
        ...
        $('<div id="post-date"></div>').prependTo(user_notes);
        ...
        posts.first().find( 'a[href^="http://forums.frontier.co.uk/showthread.php"]' ).each(function() {
            ...
            $.ajax({
                ...
                success: function(html) {
                    ...
                    ;
                    $html
                        .find( '#post_message_' + post.href.replace( /.*#post/, '' ) ).next().find('a[href^="posthistory.php?"]').parent()
