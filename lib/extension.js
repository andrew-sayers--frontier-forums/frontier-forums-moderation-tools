BabelExt.utils.dispatch(
     ...
     */
    {
        ...
        callback: function( stash, pathname, params, infractions, infractions_timestamp ) {
...

    {
        ...
        callback: function( stash, pathname, params, infractions, infractions_timestamp ) {
            ...
            if ( (infractions_timestamp||0) < new Date().getTime() ) {
                $.ajax({
                    url: '/infraction.php?do=report&u=123', // get any old infraction page to find the infractions list
                    dataType: 'html',
                    success: function(html) {
                        stash.infractions = $(html).find('input[name="infractionlevelid"]').map(function() {
                            if ( $(this).val() != '0' )
                                return {
                        ...
                        BabelExt.storage.set( 'infractions', JSON.stringify( stash.infractions ) );
                        BabelExt.storage.set( 'infractions_timestamp', JSON.stringify( new Date().getTime() + 60*60*1000 ) );
                    },
                    error: function() {
     ...
     * many variables are stored on the site: /showthread.php?t=32626
     *
     ...
     */
    {
        ...
        callback: function( stash, pathname, params, variables, variables_timestamp ) {

            variables = JSON.parse(variables||'{}');

            if ( (variables_timestamp||0) < new Date().getTime() ) {
                stash.get_thread_posts( 32626 ).then(
                    function(posts) {
                        return $.when.apply(
                            $,
                            ...
                            BabelExt.storage.set( 'variables_timestamp', JSON.stringify( new Date().getTime() + 60*60*1000 ) );
                        });
                    },
                    function() {
            ...
            if ( stash.watched_users_timestamp < new Date().getTime() ) {
                $.ajax({
                    ...
                    success: function( msg ) {
                        stash.watched_users = {};
                        $(msg).find( '#post_message_246029 a[href^="/search.php?do=finduser&u="]' )
                            .each(function() {
