            ...
            stash.log_in_to_modcp = function( selector, page ) {
                ...
                iframe.one( 'load', function() {
                    if ( $('#vb_login_username', iframe[0].contentDocument.body ).length ) {
                        iframe.css({ display: 'block', width: '450px', height: '196px' });
                        $(iframe[0].contentDocument.body).find('p').remove();
                        var interval = setInterval(function() {
                            if ( !$('#redirect_button,#vb_login_username', iframe[0].contentDocument.body).length ) {
                                iframe.hide();
                        ...
                        dfd.resolve(iframe[0]); // already logged in
                        return;
                    }
                });
                return dfd.promise();
