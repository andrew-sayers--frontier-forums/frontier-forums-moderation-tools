BabelExt.utils.dispatch(
     ...
     */
    {
        ...
        callback: function( stash, pathname, params, infractions ) {
            ...
            /*
             * Return a list of links to common spam-detection sites
             */
            ...
            /*
             * Security improvements for ModCP are afoot.  This function has been added so we can react more quickly when the time comes.
             */
            ...
            */
            ...
            stash.get_user_ips = function(username, get_overlapping) {
                if ( !get_user_ips_data )
                    return $.get( stash.modcp_url( 'user.php?do=doips' ) ).then(function(html) {
                        html = $(html);
                        get_user_ips_data = {
                            'do': 'doips',
                ...
                return $.post( stash.modcp_url( 'user.php?do=doips' ), $.extend( get_user_ips_data, { username: username, depth: get_overlapping ? 2 : 1 } ) ).then(function(html) {
                    html = $(html);
                    var ret = {
            ...
            stash.get_ip_data = function(ip) {
                ...
                return $.post( stash.modcp_url( 'user.php?do=doips' ), $.extend( get_user_ips_data, { ipaddress: ip, depth: 1 } ) ).then(function(html) {
                    html = $(html);
                        ...
                        var interval = setInterval(function() {
                            if ( $('#vb_login_username', iframe[0].contentDocument.body ).length ) {
                                ...
                                interval = setInterval(function() {
                                    if ( $( '.copyright', iframe[0].contentDocument.body ).length ) {
        ...
        callback: function( stash, pathname, params, variables, variables_origin, variables_timestamp, user_language, reload_interval ) {
            ...
            function get_variables_thread( thread_datum, first_page ) {
                function convert_replies() {
                    return stash.when( thread_datum.replies.map(function(post_id) { return stash.get_post_info(post_id) }) ).then(function(posts) {
                        thread_datum.variables = {};
                        posts.forEach(function(post) {
                            stash.process_quotes(post.vbcode).forEach(function(variable) {
...

    {
        ...
        callback: function( stash, pathname, params, infractions, infractions_timestamp, variable_thread_id, reload_interval ) {
            ...
            stash.ban_user = function( user, message, is_spam, period, expires ) {
                return stash.post_message( stash.modcp_url( 'banning.php?do=dobanuser' ), {
                    'do': 'dobanuser',
                    username: user,
...

    {
        ...
        callback: function( stash, pathname, params, openclose_button) {
            ...
            $(
                ...
                .appendTo(li)
                .click(function(event) {
                    stash.thread_openclose( params.t, $(this).attr('href') == '#open' ).done(function() {
                        location.reload();
                    });
                    event.preventDefault();
                })
                ...
                var form = $(
                    '<div class="row">' +
                      ...
                      '<div class="column2">' +
                        ...
                        // April fools!
                        ( new Date().getMonth() == 0 && new Date().getDate() == 28 ? '<input type="button" class="april-fools" value="BAN THEM ALL!">' : '' ) +
                ...
                $('#submit-resolution').click(function(event) {
                    if ( !action_count ) {
                        stash.thread_reply_and_go(
                            location.toString(),
                            params.t,
                            stash.parse_variable('report process', 'post title: close report without action', {}, 'string', forum_to_review_id, stash.thread_to_review_id),
                            stash.parse_variable('report process', 'post body: close report without action' , {}, 'string', forum_to_review_id, stash.thread_to_review_id),
                            !thread_closed
                        );
                        event.preventDefault();
                        return;
                ...
                $('.april-fools').click(function() {
                    ...
                    setInterval(function() {
                        ...
                        $.get('/member.php?u='+user_id+')').then(function(html) {
                            ...
                            if ( !name ) return;
                            var ban = $('<div>' + BabelExt.utils.escapeHTML(name) + '... ' + stash.loading_html + '</div>').prependTo( box );
                            setTimeout(function() {
