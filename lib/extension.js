(function(localStorage) { // note: we pass in a 'localStorage' variable because Firefox extensions can't use the real localStorage directly
    ...
    function copy_to_user_note() {
        ...
        $('#copy_to_user_note').prop( 'checked', check_user_note_checkbox || false );
    }
    ...
    posts.each(
        function() {
            ...
            $('<tr><td class="vbmenu_option vbmenu_option_alink"><a href="http://www.stopforumspam.com/search?advip=on&q=' + $('img[alt=IP]', this).attr('title') + '">Search for IP address on Stop Forum Spam</a></td></tr>' +
              '<tr><td class="vbmenu_option vbmenu_option_alink"><a href="http://www.stopforumspam.com/search?advusername=on&q=' + encodeURIComponent(username) + '">Search for username on Stop Forum Spam</a></td></tr>')
    ...
    if ( $('table.tborder').eq(0).has( 'a[href="forumdisplay.php?f=48"]' ).length ) {
        ...
        posts.first().find( 'a[href^="http://forums.frontier.co.uk/member.php"]' ).each(function() {
            if ( reported_user != this.getAttribute( 'href' ).split( '?u=' )[1] ) { // people sometimes report their own posts
                ...
                var notes =
                    $('<div style="border-left: 3px solid black; padding-left: 1em"><h1>Notes for ' + $(this).html() + '</h1><div class="activity"></div><h2>Infractions for ' + $(this).html() + '</h2><div class="infractions"></div><br><div class="user-notes"></div></div>' )
                ...
                ;
                notes.find('.infractions')
                    .load( this.getAttribute( 'href' ) + ' #collapseobj_infractions', function() {
                        $(this).removeAttr( 'id' );
                        if ( $(this).find('tr').length > 1 ) {
                            ...
                            $(this).html( '(no infractions)' ).prev().remove();
                        }
                    });
                notes.find('.activity')
                    .load( this.getAttribute( 'href' ) + ' #activity_info', function() {
                        $(this).find('.alt2').removeClass('alt2');
                    });
            }
        });
    ...
    $('img[src="images/misc/moderated.gif"]').wrap('<a href="http://forums.frontier.co.uk/modcp/moderate.php?do=posts"></a>')
